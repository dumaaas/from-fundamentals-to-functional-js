(function () {
    ...
    describe('Scope Exercise', function () {
        ...
        it('block scope can be created with let', function () {
            var fn = function () {
                var where = 'outer'
                // let creates it owns block 
                // if it it was war -> expect ACTUAL would be inner
                // var do not create it own block
                {
                    let where = 'inner'
                }
                ACTUAL = where;
        ...
        it('a function\'s local scope variables are not available anywhere outside that function', function () {
            ...
            firstFn();
            // we are expecting to throw an error becouse localToFirstFn is local variable and localToFirstFn in this block is undefined
            expect(function () {
        ...
        it('a function\'s local scope variables are not available anywhere outside that function, regardless of the context it\'s called in', function () {
            var firstFn = function () {
                ...
                // Although false, it might seem reasonable to think that the secondFn (which mentions the localToFirstFn variable), should have access to the 
                // localToFirstFn variable, since it's being called here from within the scope where that variable is declared.
                secondFn();
            }
            ...
            expect(function () {
                // in addition, calling the firstFn (which in turn calls the secondFn) should also throw, since it the calling context of secondFn has no 
        ...
        it('a new variable scope is created for every call to a function, as exemplified with a counter', function () {
            var fn = function () {
                // the `||` symbol here is being used to set a default value for innerCounter. If
                // innerCounter already contains a truthy value, then the value in that variable will be
                // unchanged. If it is falsey however (such as if it were completely uninitilizied), then 
                // this line will set it to the default value of 10.
        ...
        it('a new variable scope is created for each call to a function, as exemplified with uninitialized string variables', function () {
            ...
            var fn = function () {
                ...
                // now that acutal has been set, we will initialize localVariable to refer to a string
        ...
        it('an inner function cann access both its local scope variables and variables in its containing scope, provided the variables have different names', function () {
            var outerName = 'outer';
            var fn = function () {
                var innerName = 'inner';
        ...
        it('the rules about retaining access to variables from an outer scope still applies, even after the outer function call (that created the outer scope) has returned', function() {
            ...
            // running this outer function should have the same effect as running the whole previous test, with the addition of placing the innerFn somewhere 
            // that we can reach it after outerFn has returned
            outerFn();
            expect(window.retainedInnerFn).to.be.a('function');
            // even though the outerFn has returned once the only call to it was completed a couple of lines above, the inner function remains
            // available in the global scope, and still has access to the variables of that containing scope where it was first created.
